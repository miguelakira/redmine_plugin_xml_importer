...
module LoaderHelper
  ...
  def loaderhelp_user_selector( fieldId, project, parents, task )
    # @tasks_resources = {task_uid => colaborador}
    # @colaboradores = {resource_uid => colaborador}

    # se a tarefa nao for o ultimo no, ela deve ser atribuida ao Pacote de Trabalho 
    if parents.include?(task.outlinenumber)  
    # First populate the selection box with all the existing categories from this project
      memberList = Member.find( :all, :conditions => ["project_id = ? and user_id != ?", project, 43] )
    else
    ...
    # Pacote de Trabalho caso seja pai de alguma tarefa
    if parents.include?(task.outlinenumber)
      output << "<option value=\"43\">Pacote de Trabalho</option>"
    # Insere o nome dos colaboradores
    elsif !@tasks_resources[task.uid].nil?
      output << "<option value=\"#{@tasks_resources[task.uid].id}\">#{@tasks_resources[task.uid].name}</option>"
