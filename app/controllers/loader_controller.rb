...
class LoaderController < ApplicationController
    ...
    unless ( xmlfile.nil? )
      ...
      begin
        ...
        if ( @import.tasks.nil? or @import.tasks.empty? )
          ...
          flash[ :notice ] = 'Tarefas lidas corretamente. Selecione os itens para importar.'
        ...
        Issue.transaction do
          to_import.each do | source_issue |
            ...
            if (source_issue.milestone.to_i == 0) 
              ...
              destination_issue.custom_field_values.each do |cf|
                if cf.custom_field_id == 9
                  cf.value = '1'
                end
              end
    ...
    xml.Project do    
       xml.Tasks do
         xml.Task do  
           ...
           xml.Type("1")
           xml.CreateDate(@project.created_on)
         ...
         issues.each do |issue|
           xml.Task do
             ...
             xml.CreateDate(issue.created_on)
             xml.Priority(issue.priority_id)
             xml.Start(issue.start_date.to_s().concat("T08:00"))
             xml.Finish(issue.due_date.to_s().concat("T17:00"))
             xml.FixedCostAccrual("3")
             xml.ConstraintType("4")
             xml.ConstraintDate(issue.start_date.to_s().concat("T08:00"))
         ...
         versions.each do |version|
           xml.Task do
             ...
             xml.CreateDate(version.created_on)
             xml.Start(version.effective_date.to_s().concat("T08:00"))
             xml.Finish(version.effective_date.to_s().concat("T08:00"))
             xml.FixedCostAccrual("3")
             xml.ConstraintType("4")
             xml.ConstraintDate(version.effective_date.to_s().concat("T08:00"))
  ...
  def get_tasks_from_xml( doc )
    ...
    doc.each_element( 'Project/Tasks/Task' ) do | task |
      begin
        ...
        struct.duration = task.get_elements( 'Duration' )[ 0 ].text
 ...
 #########################
       ...
       struct.work = task.get_elements( 'Work' )[ 0 ].text
       hours = 0
       mins = 0
       secs = 0
       ...
       #  Pega o tempo estimado da tarefa e faz o parseamento da string PT<num>H<num>M<num>S. 
...
###########################
    ...
    # Comentei para evitar que apaguem as tasks propositalmente duplicadas
    tasks = tasks.uniq
    ...
    ###################
    # Pega todos os UIDs e respectivos nomes dos colaboradores
    @resources = {}
    doc.each_element( 'Project/Resources/Resource' ) do | res |
      @resources[res.get_elements( 'UID' )[0].text.to_i] = res.get_elements( 'Name' )[0].text.to_s unless (res.get_elements( 'Name' )[0]).nil?
    end
    
    @colaboradores = {}
    @resources.each do |res|
      # busca por firsname
      lista = User.find_all_by_firstname(res[1].split.first.titleize)
      if lista.count == 1
        @colaboradores[res[0]] = lista[0]
      elsif lista.count > 1
        if res[1].split.count > 1
          @colaboradores[res[0]] = User.find_by_lastname(res[1].split.last.titleize)
    ...
    @tasks_resources = {}
     #vai pegar os UIDs das tasks e associar com os UIDs dos @colaboradores
     doc.each_element( 'Project/Assignments/Assignment' ) do | as |
      @tasks_resources[as.get_elements( 'TaskUID' )[0].text.to_i] = @colaboradores[as.get_elements( 'ResourceUID' )[ 0 ].text.to_i] unless (as.get_elements( 'ResourceUID' )[0]).nil?
    end
    ################
    ...
    #end
########
  # Alterações feitas para caso uma tarefa tenha mais de um colaborador, duplicá-la. 
        
  
    # Verifica as tarefas que possuem diferença entre work e duration
     tasks_to_verify = []
     predecessors = []
     last_outlinenumber = []
     tasks.each do |task|
       resources = task.work / task.duration unless task.duration.nil?
        if resources > 1
          # é preciso que o duration seja divisão exata de work, por ex, 36/6 = 6 colaboradores com 6 horas cada.
          if resources == resources.floor
              resources = resources
              # executa o push x vezes, dependendo do valor da divisão acima. Por exemplo, se 48/4 = 12, 
              # serao executados 12 pushs no array de tarefas
              resources.to_i.times do |x|

                tasks_to_verify.push(task)
              end
              predecessors.push(task.outnum)
          end
        end
     end
     
     
     # Verifica se as tarefas são o último nó da árvore
     tasks_to_duplicate = []
     tasks_to_verify.each do |task|
        unless predecessors.include?(task.outlinenumber)
          tasks_to_duplicate.push(task)
        end
     ...
     # Aqui faz a duplicação de fato. Clona a tarefa e muda a UID da tarefa.
     tasks_to_duplicate.each_index do |x|
         struct_duplicated = OpenStruct.new
         struct_duplicated = tasks_to_duplicate[x].clone
         #temp = (struct_duplicated.outlinenumber[-1,1].to_i + 20).to_s
          
          #struct_duplicated.outlinenumber[-1,1] = temp
          novo_outlinenumber = struct_duplicated.outlinenumber
          novo_outlinenumber = novo_outlinenumber.to_s
          novo_outlinenumber = novo_outlinenumber + ".1"
          struct_duplicated.outnum = struct_duplicated.outlinenumber
          struct_duplicated.outlinenumber = novo_outlinenumber
          struct_duplicated.level = struct_duplicated.level + 1
          

          struct_duplicated.uid = last_uid + x + 1
          
          
          tasks.push( struct_duplicated )
...
###############
    ...
    real_tasks = real_tasks.sort_by { | task | task.outlinenumber }
